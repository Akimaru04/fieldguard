...
<div class="flex h-screen w-screen">
    <aside class="w-64 border-r border-slate-200 bg-white flex-shrink-0"><?php include __DIR__ . '/../includes/sidebar.php'; ?></aside>
        ...
        <div class="max-w-6xl mx-auto w-full">
            ...
            <div class="flex justify-between items-center mb-6">
                ...
                <?php if ($_SESSION['role'] === 'Admin'): ?>
                    <a href="/admin/export.php" class="bg-emerald-600 text-white px-5 py-2 rounded-lg text-sm font-bold hover:bg-emerald-700 transition">
                        Export CSV
                    </a>
                <?php endif; ?>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                    <h3 class="text-slate-400 text-sm font-bold uppercase">Total Logs</h3>
                ...
                <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                    <h3 class="text-slate-400 text-sm font-bold uppercase">Active Shifts</h3>
                    <p class="text-4xl font-bold mt-2"><?= $pdo->query("SELECT COUNT(*) FROM attendance_logs WHERE check_out_time IS NULL")->fetchColumn() ?></p>
                </div>
            </div>
            ...
            <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm mb-8">
                ...
                <table class="w-full text-sm text-left">
                    <thead><tr class="text-slate-400 border-b"><th>Worker</th><th>Shift</th><th>Check-in</th></tr></thead>
                    <tbody>
                        <?php
                        $stmtActive = $pdo->query("SELECT u.name, l.shift_type, l.check_in_time FROM attendance_logs l JOIN users u ON l.user_id = u.id WHERE l.check_out_time IS NULL AND l.check_in_time = (SELECT MAX(check_in_time) FROM attendance_logs WHERE user_id = u.id AND check_out_time IS NULL)");
                        while($log = $stmtActive->fetch(PDO::FETCH_ASSOC)): ?>
                            <tr class="border-b"><td class="py-3"><?= e($log['name']) ?></td><td class="py-3"><?= e($log['shift_type']) ?></td><td class="py-3 text-slate-500"><?= $log['check_in_time'] ?></td></tr>
                        <?php endwhile; ?>
                    </tbody>
                </table>
            </div>
            ...
            <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm mb-8">
                ...
                if ($stmtLogs->rowCount() === 0): ?>
                    <p class="text-sm text-slate-400 italic">No flagged activity.</p>
                <?php else: ?>
                    <table class="w-full text-sm text-left">
                        ...
                        <tbody>
                            <?php while($log = $stmtLogs->fetch(PDO::FETCH_ASSOC)): ?>
                                <tr class="border-b"><td class="py-3"><?= e($log['name']) ?></td><td class="py-3"><?= e($log['site_name']) ?></td><td class="py-3 text-red-600 font-bold"><?= e($log['status']) ?></td></tr>
                            <?php endwhile; ?>
                        </tbody>
                    </table>
                <?php endif; ?>
